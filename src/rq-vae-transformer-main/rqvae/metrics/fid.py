...
def compute_rfid(dataset,
    ...
    batch_cnt = 0
    img_cnt = 0
    saveReconstructImgPath = "/root/luke/codes/rq-vae-transformer-main/output_pic"
    lpips_model = lpips.LPIPS(net='vgg').cuda()
    Dists = DISTS().cuda()
    dists_avg = 0
    lpips_avg = 0

    for xs, _ in tqdm(loader, desc="compute PSNR"):
        batch_cnt += 1
        ...
        with torch.no_grad():
            #x_hat, _, _ = stage1_model(xs)
            #x_hat = []
            #for i in range(xs.shape[0]):
            #x = xs[i:i+1]
            z_e = stage1_model.module.encode(xs)
            # codeBook = stage1_model.module.quantizer.get_all_codebook_weights()
            # mask = stage1_model.module.indice_num_predict(xs, z_e_list, lamda, stage)
            z_q, quant_loss, code = stage1_model.module.quantizer(z_e)

            # xs_recon = torch.cat([
            #     stage1_model(z_e[i:i+1], lamda, stage)[0] for i in range(z_e.shape[0])
            # ], dim=0)
            # print(code)
            # z_q[:, 2, :, :] = 0
            # z_q = z_e
            # z_q[:, location[0], location[1], :] += 1
            # print(z_q.cpu().shape)
            x_hat = stage1_model.module.decode(z_q)
            # output = stage1_model.module.compute_loss(x_hat, quant_loss, code, xs=xs, 
            #                                           valid=True,)
                #x_hat.append(x_hat_temp)
            #x_hat = torch.cat(x_hat, dim=0)

            Lpips = lpips_model(xs, x_hat)
            dists = Dists(xs, x_hat)

        lpips_avg += Lpips.mean().item()
        dists_avg += dists.mean().item()

        x_hat = x_hat.cpu()
        #x_hat = torch.clamp(x_hat*0.5 + 0.5, 0, 1)
        #x_hat = 2.* x_hat - 1
        x_hat = x_hat.numpy()
        ...
        for i in range(x_hat.shape[0]):
            reconstructed_img = (x_hat[i].clip(0, 1) * 255).astype(np.uint8)
            origin_img = (xs[i].clip(0, 1) * 255).astype(np.uint8)
            
            # 确保颜色空间转换
            reconstructed_img = cv2.cvtColor(reconstructed_img, cv2.COLOR_RGB2BGR)
            origin_img = cv2.cvtColor(origin_img, cv2.COLOR_RGB2BGR)
            
            psnr = cv2.PSNR(origin_img, reconstructed_img)           

            psnr_avg += psnr

            img_cnt += 1

            concatenated_img = cv2.hconcat([origin_img, reconstructed_img])
            if location:
                reconstructed_img_path = os.path.join(saveReconstructImgPath, f'reconstructed_{img_cnt}_{location[0]}{location[1]}.jpg')
            else:
                reconstructed_img_path = os.path.join(saveReconstructImgPath, f'reconstructed_{img_cnt}.jpg')
            cv2.imwrite(reconstructed_img_path, concatenated_img)
    # rfid=3.1999
    return rfid, psnr_avg/img_cnt, lpips_avg/batch_cnt, dists_avg/batch_cnt
