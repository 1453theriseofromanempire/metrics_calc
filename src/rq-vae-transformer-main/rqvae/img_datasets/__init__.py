...
def create_dataset(config, is_eval=False, logger=None):
    ...
    if config.dataset.type == 'imagenet':
        ...
        # root = root if root else '/root/autodl-tmp/imagenet'
        # # val_root = '/root/luke/data/CLIC'
        # train_path = f"{root}/train"
        # # val_path = f"{root}/valid"
        # val_path = "/root/autodl-tmp/CLIC"
        # dataset_trn = datasets.ImageFolder(root=train_path, transform=transforms_trn)
        # dataset_val = datasets.ImageFolder(root=val_path, transform=transforms_val)
        root = root if root else '/root/autodl-tmp/ILSVRC2012_train'
        # val_root = '/root/luke/data/CLIC'
        train_path = f"{root}/data"
        val_path = "/root/autodl-tmp/clic"
        dataset_trn = datasets.ImageFolder(root=train_path, transform=transforms_trn)
        dataset_val = datasets.ImageFolder(root=val_path, transform=transforms_val)
