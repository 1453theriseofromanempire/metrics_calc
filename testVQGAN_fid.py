...
cfg = "/root/home/codes/imp_trans/src/taming-transformers/configs/imagenet_vqgan.yaml"
...
def eval(model, config, root_path, generated_image_root_path):
    ...
    for subfolder in subfolders:
        ...
        dataset_val = create_dataset(subfolder_path)
...
def main():
    ...
    checkpoint = torch.load('/root/autodl-tmp/ckpts/last.ckpt')
    ...
    output_not_shift = eval(model, config, root_not_shift_path, generated_image_root_not_shift_path)
    ...
    ############### calc metrics ###########################################
    total_num = output_not_shift['total_num'] + output_shift['total_num']
    ...
    print(f"PSNR: {psnr} | LPIPS: {lpips} | FID: {fid}")
